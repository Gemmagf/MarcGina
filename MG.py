...
st.markdown("<h1 style='text-align: center; color: darkcyan;font-family:verdana;font-size:300%;'>Marc i Gina</h1>", unsafe_allow_html=True)
...
st.write("# ")
fig5 = px.scatter(filter(da,any,mes), x='Date' , y='N_words', color = 'User',color_discrete_map={'Gina': '#D0D3DA', 'Marc':'darkcyan' }, size = 'N_words',width=770,height=400)
...
st.write("# ")
st.pyplot(plot_active_hours(filter(da,any,mes), color='darkcyan', savefig=False, dpi=100, user='Marc'))
st.pyplot(plot_active_hours(filter(da,any,mes), color='#D0D3DA', savefig=False, dpi=100, user='Gina'))
...
wordcloud = WordCloud(width = 1000, height = 400, background_color ='white', stopwords = stop_words, colormap='YlGnBu',min_font_size = 10).generate(comment_words)   
